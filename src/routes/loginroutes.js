...
const cookdb=require('/home/haritha/kba/MONGO_DB/Cookbook app with database/models/schema.js')
...
loginroute.post('/signup', async(req, res) => {
    ...
    //     if(cookdb.findOne({username:username})){
    //     res.send("User already exists. Click login")
    // }else{
    //     // cookdb[username]= {email: email, password: password, favourites: []}
    //     const data=await cookdb.create(newuser)
    //     req.session.username = username;
    //     req.session.favourites =    cookdb[username].favourites;
    //     res.redirect('/home');
    // }    
    }
    catch (error) {
